...
> [!TIP]
> **Personal multi-platform setup**: Set `"unified_session": true` under `agents.defaults` to share one session, memory, and context across all channels. You can chat on DingTalk and pick up on Feishu — the bot remembers everything.
...
> `streaming`: Defaults to `false`. Enable to buffer progressive output into a single markdown message (DingTalk does not support native message editing). Tool hints are merged inline when streaming is active.
