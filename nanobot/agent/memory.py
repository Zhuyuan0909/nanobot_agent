...
class Dream:
    ...
    async def run(self) -> bool:
        ...
        # Refresh file contents before Phase 2 — the agent may have modified
        # MEMORY.md via the memory skill during the Phase 1 LLM call.
        raw_memory_2 = self.store.read_memory() or "(empty)"
        current_memory_2 = (
            self._annotate_with_ages(raw_memory_2)
            if self.annotate_line_ages
            else raw_memory_2
        )
        current_soul_2 = self.store.read_soul() or "(empty)"
        current_user_2 = self.store.read_user() or "(empty)"

        refreshed_file_context = (
            f"## Current Date\n{current_date}\n\n"
            f"## Current MEMORY.md ({len(current_memory_2)} chars)\n{current_memory_2}\n\n"
            f"## Current SOUL.md ({len(current_soul_2)} chars)\n{current_soul_2}\n\n"
            f"## Current USER.md ({len(current_user_2)} chars)\n{current_user_2}"
        )
        ...
        phase2_prompt = f"## Analysis Result\n{analysis}\n\n{refreshed_file_context}{skills_section}"
