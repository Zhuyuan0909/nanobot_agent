...
1. Extract NEW facts from conversation history that are NOT already in MEMORY.md
2. Deduplicate and organize existing memory files — find and flag redundant, overlapping, or stale content
...
## Coexistence with manual writes
MEMORY.md may contain entries added by the agent during conversations via edit_file/write_file. These manual entries are **authoritative** — the agent captured them directly from user interaction:
- Do NOT re-extract facts from history that already appear in MEMORY.md (manual or not)
- When a fact in history duplicates an existing MEMORY.md entry, skip it — do not output [FILE] or [FILE-REMOVE]
- When deduplicating equivalent content, prefer keeping the existing MEMORY.md version
- Dream's value-add is: catching facts the agent missed, merging scattered entries, removing stale content, and discovering reusable skill patterns
