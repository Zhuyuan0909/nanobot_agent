...
class DingTalkConfig(Base):
    ...
    streaming: bool = False  # Buffer deltas & send complete markdown on stream end
...
class DingTalkChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        # Streaming buffer: accumulate deltas per chat_id, send on _stream_end
    ...
    async def _send_batch_message(
        ...
        try:
            ...
            try: result = resp.json()
                ...
                logger.exception("DingTalk failed to parse response JSON, body={}", body[:500])
                result = {}
    ...
    async def send_delta(
        ...
        """Buffer streaming deltas, send complete markdown on _stream_end.

        DingTalk robot API does not support progressive message editing,
        so we accumulate deltas and deliver the final markdown at the end.
        """
        meta = metadata or {}
        ...
        if meta.get("_stream_end"):
            ...
            if not token:
                return
            await self._send_markdown_text(token, chat_id, buf.strip())
            return

        # --- accumulate delta ---
        buf = self._stream_bufs.get(chat_id, "")
        self._stream_bufs[chat_id] = buf + delta
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        # Route tool hints into the stream buffer when streaming is active,
        # so they appear inline with the progressive output.
        if msg.metadata.get("_tool_hint"):
            ...
            if buf is not None:
                # Streaming is active — inline the hint
                self._stream_bufs[msg.chat_id] = buf + f"\n\n---\n> {hint}\n\n"
                return
            # No active stream — send as standalone markdown
            await self._send_markdown_text(token, msg.chat_id, f"> {hint}")
            return
